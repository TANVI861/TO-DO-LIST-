...
class TaskManager:
    ...
    def create_widgets(self):
        ...
        Button(
            self.functions_frame,text="Add",width=15,bg="#D4AC0D",font=("arial","14","bold"),command=self.add_task
        ).place(x=18,y=80)

        Button(
            self.functions_frame,text="Remove",width=15,bg="#D4AC0D",font=("arial","14","bold"),command=self.delete_task
        ).place(x=240,y=80)

        Button(
            self.functions_frame,text="Delete All",width=15,bg="#D4AC0D",font=("arial","14","bold"),command=self.delete_all_tasks
        ).place(x=460,y=80).

        Button(
            self.functions_frame,text="Exit / Close",width=52,bg="#D4AC0D",font=("arial","14","bold"),command=self.close
        ).place(x=17,y=330)    

        self.task_listbox = Listbox(
            self.functions_frame, width=70,height=9,font="bold",selectmode="Single",bg="WHITE",fg="BLACK",selectbackground="#FF8C00",selectforeground="BLACK"
        )
